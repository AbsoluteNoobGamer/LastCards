...
void writeBgmWav(String path, double durationSecs, double maxVol) {
  ...
  void addNote(double freq, double startTimeSec, double noteVol) {
    ...
    for (int i = 0; i < sampleRate * 2; i++) { // Note rings for 2s max
  ...
  for (int bar = 0; bar < 4; bar++) { // 4 bars = 8 seconds total
    for (int beat = 0; beat < sequence.length; beat++) {
      final startTime = bar * sequence.length * beatDuration / 2 + beat * beatDuration / 2;
  ...
  for (int i = 0; i < numSamples; i++) {
    ...
    if (i > numSamples - 4410) { // last 100ms
