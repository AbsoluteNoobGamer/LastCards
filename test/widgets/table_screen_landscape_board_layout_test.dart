...
void main() {
  ...
  GameState seededState(int totalPlayers) {
    ...
    return GameState(
      sessionId: 'landscape-board-$totalPlayers',
  ...
  for (final playerCount in [2, 7]) {
    testWidgets(
      'landscape board has no flex overflow at 844x390 ($playerCount players)',
      (tester) async {
        await pumpTable(tester, totalPlayers: playerCount);
