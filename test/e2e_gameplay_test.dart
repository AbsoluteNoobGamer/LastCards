...
void main() {
  group('E2E Gameplay Flow Engine Test', () {
    test('Simulate complete 2-player game to win condition', () {
      ...
      List<CardModel> drawFactory(int n) {
        if (drawPile.length < n) {
          ...
          for (int i = 0; i < n; i++) {
             drawPile.add(CardModel(id: 'mock_\$i', suit: Suit.spades, rank: Rank.two));
          }
        }
      ...
      while (state.phase == GamePhase.playing && turns < maxTurns) {
        ...
        for (final p in state.players) {
           expect(p.hand.length, greaterThanOrEqualTo(0), reason: 'Player hand count cannot be negative');
        }
      ...
      // 5. Assertions
      expect(state.phase, GamePhase.ended, reason: 'Game must finish within a reasonable turn limit. (Failed after \$turns turns)');
      ...
      expect(winner.hand.isEmpty, isTrue, reason: 'Winner must have 0 cards left');
    });
    ...
    test('Regression scenario: Trying to play invalid card returns error', () {
      ...
      final invalidCard = const CardModel(id: 'invalid_card', suit: Suit.spades, rank: Rank.nine);
      
      // Set discard to 4 of Hearts
      state = state.copyWith(discardTopCard: const CardModel(id: '4h', suit: Suit.hearts, rank: Rank.four));
      
      final err = validatePlay(cards: [invalidCard], discardTop: state.discardTopCard!, state: state);
      
      // Assert engine securely blocks invalid move
      expect(err, isNotNull, reason: 'Must reject card with no matching suit/rank');
      expect(err, contains('Must match'), reason: 'Proper error message is returned');
    });
  });
}
