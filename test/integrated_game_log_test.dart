...
void main() {
  ...
  testWidgets('Normal play log shows correct formatting', (WidgetTester tester) async {
  ...
  testWidgets('Draw action log shows correct formatting', (WidgetTester tester) async {
    final entry = MoveLogEntry(
      player: 'Player 2',
      isDraw: true,
      drawCount: 3,
    ...
    expect(find.textContaining('drew 3 cards (penalty)', findRichText: true), findsOneWidget);
  });

  testWidgets('Joker play log explicitly shows effective state', (WidgetTester tester) async {
    var jokerCard = const CardModel(id: 'j1', rank: Rank.joker, suit: Suit.spades);
    jokerCard = jokerCard.copyWith(jokerDeclaredRank: Rank.seven, jokerDeclaredSuit: Suit.diamonds);
    ...
    expect(find.textContaining('→ Seven of Diamonds', findRichText: true), findsOneWidget);
  });

  testWidgets('Multi-card play log shows correct formatting', (WidgetTester tester) async {
    final entry = MoveLogEntry(
      player: 'Player 4',
      cards: [
  ...
  testWidgets('Multi-turn behavior maintains scroll state', (WidgetTester tester) async {
    final entries = List.generate(15, (i) => MoveLogEntry(
      player: 'P$i',
  ...
  testWidgets('PlayerStyles correctly inject table position icons and colors', (WidgetTester tester) async {
    final entry = MoveLogEntry(
      player: 'Opponent',
    ...
    for (final text in richTexts) {
      if (text.text.toPlainText(includeSemanticsLabels: false).contains('Opponent')) {
        targetText = text;
        break;
      }
    }
    
    expect(targetText, isNotNull, reason: 'Could not find RichText containing "Opponent"');
